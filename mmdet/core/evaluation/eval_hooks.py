...
class CocoLocalEvalmAPHook(LocalEvalHook):
...
class CocoLocalEvalRecallHook(LocalEvalHook):
    def __init__(self,
                 dataset,
                 interval=1,
                 proposal_nums=(100, 300, 1000),
                 iou_thrs=np.arange(0.5, 0.96, 0.05)):
        super(CocoLocalEvalRecallHook, self).__init__(dataset, interval=interval)
    ...
    def evaluate(self, runner, results):
        ...
        ar = fast_eval_recall(results, self.dataset.coco, 
                              self.proposal_nums, self.iou_thrs)
